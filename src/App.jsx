...
const App = () => {
  ...
  const [speaker1Voice, setSpeaker1Voice] = useState('Kore'); // 話者1の音声タイプ
  const [speaker2Voice, setSpeaker2Voice] = useState('Puck'); // 話者2の音声タイプ
  ...
  const generateRawStructure = useCallback(() => {
    ...
    // スタイル指示を含むプロンプトを構築
    let prompt = 'Read aloud in a warm, welcoming tone\n'; // デフォルトの指示
    ...
    if (speaker1StyleText || speaker2StyleText) {
      ...
      if (speaker1StyleText) styleInstructions.push(`Make ${speaker1Name} sound ${speaker1StyleText}`);
      if (speaker2StyleText) styleInstructions.push(`Make ${speaker2Name} sound ${speaker2StyleText}`);
      prompt += `Style instructions: ${styleInstructions.join(', ')}\n\n`;
    }
    ...
    setError('');
  }, [getConversationText, speaker1Name, speaker2Name, speaker1Styles, speaker2Styles]);
  ...
  const generateAudio = async () => {
    ...
    try {
      ...
      if (speaker1StyleText || speaker2StyleText) {
        ...
        if (speaker1StyleText) styleInstructions.push(`Make ${speaker1Name} sound ${speaker1StyleText}`);
        if (speaker2StyleText) styleInstructions.push(`Make ${speaker2Name} sound ${speaker2StyleText}`);
        prompt += ` with the following style: ${styleInstructions.join(', ')}`;
      }
      prompt += ` between ${speaker1Name} and ${speaker2Name}:\n\n${conversationText}`;
      ...
      if (data.candidates?.[0]?.content?.parts?.[0]?.inlineData?.data) {
        ...
        throw new Error('音声データが正常に生成されませんでした。レスポンスの構造を確認してください。');
      }
    } catch (err) {
  ...
  const downloadAudio = () => {
    if (audioUrl) {
      ...
      // ダウンロード後に revokeObjectURL を呼ぶのは、再生もできなくなるため注意。
      // もしダウンロード後も再生させたい場合は、コンポーネントアンマウント時などにまとめて revoke する。
      // ここではダウンロード専用として即時 revoke しないでおく。再生も可能なため。
      // URL.revokeObjectURL(audioUrl); // メモリ解放 (必要に応じて)
    }
  };
  ...
  const StructuredConversationInput = () => {
    ...
    useEffect(() => {
        // speaker1Nameが変更され、かつcurrentSpeakerが以前のspeaker1Name（かつspeaker2Nameではない）場合、新しいspeaker1Nameに更新
        const oldSpeaker1Name = structuredConversation.find(item => item.speaker !== speaker2Name)?.speaker;
    ...
    useEffect(() => {
        // speaker2Nameが変更され、かつcurrentSpeakerが以前のspeaker2Name（かつspeaker1Nameではない）場合、新しいspeaker2Nameに更新
        const oldSpeaker2Name = structuredConversation.find(item => item.speaker !== speaker1Name)?.speaker;
